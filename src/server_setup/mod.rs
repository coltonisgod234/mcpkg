...
pub async fn sync(c: &Config) {
    match &c.server.kind {
        ServerKind::Fabric { fabric_version, installer_version } => {
            fabric::teardown().await;
            fabric::download_fabric_server_at(&c.server.version, &fabric_version, &installer_version).await;
            fabric::setup_server(c).await
                .expect("failed to set up server");
            fabric::plugins::apply_mod_updates(c).await;
        },
